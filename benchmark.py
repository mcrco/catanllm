...
def create_llm_player(
    color: Color, prompt: str, thinking: bool, use_mini_map: bool
) -> LLMPlayer:
...
def main():
    ...
    if not OPENROUTER_API_KEY:
        console.print(
            "[bold red]Error: OPENROUTER_API_KEY environment variable not set.[/bold red]"
        )
        return
    ...
    console.print(
        f"Model: [cyan]{MODEL}[/cyan], Games per match: [cyan]{NUM_GAMES_PER_MATCH}[/cyan], Mini Map: [cyan]{USE_MINI_MAP}[/cyan]"
    )
    ...
    llm_factories = {
        variant: get_player_factory(
            "llm", prompt=variant[0], thinking=variant[1], use_mini_map=USE_MINI_MAP
        )
    ...
    for res in all_results:
        if res["p1_avg_tokens"]:
            ...
            if name not in token_stats:
                token_stats[name] = {"reasoning": [], "response": []}
    ...
    for name, stats in sorted(fallback_stats.items()):
        ...
        fallback_table.add_row(
            name, str(fallbacks), str(decisions), f"{failure_rate:.2f}%"
        )
